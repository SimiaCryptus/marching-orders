...
*enemy* `forward` sign one cell in front of it and an enemy `arrow` on the far side pointing
...
high — one ladder kit (3 segments) covers both only if the player places the crate *after* the
