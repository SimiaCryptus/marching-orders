...
function writeStorage(key, value) {
   try { localStorage.setItem(key, value); } catch { /* private mode / quota: simply not remembered */ }
}
/**
...
function typicalGround(world) {
  ...
  for (let x = 0; x < world.w; x++) { tally(x, 0); tally(x, world.d - 1); }
  for (let z = 1; z < world.d - 1; z++) { tally(0, z); tally(world.w - 1, z); }
  let best = '', bestN = -1;
...
export class Editor {
  ...
  buildUI() {
    ...
    facingRow.append(this.facingInfo, button('Rotate [Q]', () => this.rotateFacing(1)));
    ...
    f.enemyCount = numberInput({ min: 1, max: 500 }, (v) => { this.enemyDefaults.count = v; this.applyEnemyDefaults(); });
     enemyBlock.append(field('Troops per pod', f.enemyCount));
    f.enemyRate = numberInput({ min: 0.1, max: 60, step: 0.1 }, (v) => { this.enemyDefaults.rate = v; this.applyEnemyDefaults(); });
    ...
    for (const [key, def] of Object.entries(SIGNS)) {
     ...
     campaignRow.append(f.campaign, button('Load', () => this.loadCampaignLevel(Number(f.campaign.value))));
    ...
    importRow.append(
     ...
     trackPointer(
       header,
       (e) => { if (e.target.closest('button, input, select, textarea')) return false; start = { ...this.layout }; },
   ...
   selectTab(id) {
     ...
     for (const [pid, page] of Object.entries(this.pages)) page.classList.toggle('active', pid === id);
     writeStorage(TAB_STORAGE_KEY, id);
   }
   /** Remembered panel geometry, or a default docked to the top-right of the viewport. */
   loadLayout() {
     let saved = null;
     try { saved = JSON.parse(readStorage(LAYOUT_STORAGE_KEY)); } catch { /* ignore corrupt data */ }
  ...
  syncForm() {
    ...
    f.description.value = L.description;
     this.syncGenForm();
     if (L.campaign && Number.isInteger(L.campaign.index)) f.campaign.value = String(L.campaign.index);
  ...
  updateInfo() {
    if (!this.level) return;
    const L = this.level;
    const s = L.spawn.pos, o = L.objective;
    ...
    this.guardInfo.textContent =
      `${plural(L.guards.length, 'guard')}, ${plural(L.signs.length, 'sign')}, ${plural(L.crates.length, 'crate')} placed — right-click any of them to remove it.`;
  }
  ...
  resize() {
    ...
    this.rebuildSim();
    this.syncForm();
    this.setStatus(`Resized to ${w} × ${h} × ${d}${filled ? ` — ground added under ${filled} new columns` : ''}.`);
  }
  ...
  toggleEnemySpawner(c) {
    ...
    else L.enemySpawners.push({ pos: cellArr(c), dir: dirArr(this.facing), count: this.enemyDefaults.count, rate: this.enemyDefaults.rate });
    this.rebuildSim();
  }

  signAt(c) { return this.level.signs.find((s) => sameCell(s.pos, c)) || null; }
  crateAt(c) { return this.level.crates.find((cr) => sameCell(cr.pos, c)) || null; }
  guardAt(c) { return this.level.guards.find((g) => sameCell(g.pos, c)) || null; }
  spawnerAt(c) { return this.level.enemySpawners.find((s) => sameCell(s.pos, c)) || null; }

  /** Air cell with solid ground below — where signs and crates may sit. */
  isFloor(c) {
    const w = this.world;
    return w.inBounds(c.x, c.y, c.z) && w.get(c.x, c.y, c.z) === VOXEL.AIR && w.isSolid(c.x, c.y - 1, c.z);
  }
  ...
  placeItem(c) {
    const t = this.tool, L = this.level;
    if (!this.isFloor(c)) {
    ...
    if (existing && existing.kind === t.key && existing.team === this.team) {
      ...
      if (isSign && SIGNS[t.key].directional) existing.dir = dirArr(turnRight(dirIdx(existing.dir)));
  ...
  onHover(hit) {
    ...
    if (hit.type === 'unit') {
      ...
      if (g) {
        ...
        this.setHint(`${g.def.label} at ${cellStr(g.cell)} — right-click removes it${t.kind === 'guard' ? ' (click toggles)' : ''}`);
      } else {
        r.setCursor(null);
    ...
    if (hit.type === 'sign') {
      ...
      if (same && def.directional) this.setHint(`${desc} at ${cellStr(target)} — click or wheel rotates it, right-click removes`);
      else this.setHint(`${desc} at ${cellStr(target)} — right-click removes${t.kind === 'sign' || t.kind === 'crate' ? ', click replaces' : ''}`);
      return;
    }
    ...
    switch (t.kind) {
      ...
      case 'erase': {
        ...
        this.setHint(item ? `Remove the ${item} at ${cellStr(hit.adjacent)}` : `Erase voxel at ${c}${corner}`);
        break;
      }
      case 'spawn':
        if (this.team === TEAM.ENEMY) {
          this.setHint(this.spawnerAt(target) ? `Remove the enemy pod at ${c}` : `Place an enemy pod at ${c} ${facing}`);
        } else {
          this.setHint(`Move the drop pod to ${c} ${facing}`);
        }
        break;
      case 'guard':
        this.setHint(this.guardAt(target) ? `Remove the guard at ${c}` : `Place a ${t.label.toLowerCase()} at ${c}`);
        break;
      ...
      case 'sign': {
        ...
        if (!this.isFloor(target)) {
          ...
          this.setHint(def.directional ? `Rotate the ${team} ${t.label} at ${c} (right-click removes)` : `Remove the ${team} ${t.label} at ${c}`);
        } else {
          const verb = s || this.crateAt(target) ? 'Replace with' : 'Plant';
           r.setSignPreview(t.key, target, this.facing, true);
          this.setHint(`${verb} a ${team} ${t.label} at ${c}${def.directional ? ` ${facing}` : ''} — ${describeSign(t.key, this.facing)}`);
        }
        break;
      }
      case 'crate': {
  ...
  onClick(hit, e) {
    ...
    if (hit.type === 'unit') {
      if (hit.batch === 'guards' && t.kind === 'guard' && hit.unit) this.toggleGuard(hit.unit.cell, t.type);
      this.onHover(hit);
      return;
    }
    const target = this.targetCell(hit);
  ...
  onKey(e) {
    ...
    switch (e.code) {
      ...
      case 'KeyQ': this.rotateFacing(1); return true;
      case 'KeyT': this.setTeam(otherTeam(this.team)); return true;
