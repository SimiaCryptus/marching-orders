...
export const PARAMS = Object.freeze([
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'spikes', label: 'Spike patches' },
...
export function build(p) {
  ...
  const x0 = 5,
    x1 = vx0 - 3; // leave the pod's run-up and the vault approach clear
  const yard = (x1 - x0 + 1) * (s - 2);
  const nCover = Math.round((yard * p.cover) / 100 / 3);
  for (let i = 0; i < nCover; i++) {
    const x = randInt(rng, x0, x1),
      z = randInt(rng, 1, s - 2);
    fills.push({
      type: 'dirt',
      from: [x, FLOOR_Y, z],
      to: [x, FLOOR_Y + (rng() < 0.3 ? 1 : 0), z],
    });
  }
  ...
  for (let i = 0; i < nHazards; i++) {
    const x = randInt(rng, x0 + 1, x1 - 2),
      z = randInt(rng, 1, s - 3);
    if (p.hazard === 'spikes')
      fills.push({ type: 'spikes', from: [x, FLOOR_Y - 1, z], to: [x + 1, FLOOR_Y - 1, z + 1] });
  ...
  const guards = posts
    .slice(0, Math.min(p.guards, posts.length))
    .map((g) => ({ ...g, pos: [...g.pos], dir: [-1, 0] }));
  ...
  const hazardText = nHazards
    ? `, ${plural(nHazards, p.hazard === 'spikes' ? 'spike patch' : 'pit')}`
    : '';
  const description =
