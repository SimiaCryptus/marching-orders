...
export const RULE_DEFS = Object.freeze([
  ...
  { key: 'medicRange', label: 'Medic range (cells)', min: 1, max: 16, step: 1, def: 3, group: 'Medic' },
  { key: 'medicCooldown', label: 'Seconds between heals', min: 0.1, max: 10, step: 0.1, def: 1.5, group: 'Medic' },
  // Grenades: lobbed at hostiles between the minimum and the maximum range (line of sight)
  { key: 'grenadeCharges', label: 'Grenades per crate', min: 1, max: 99, step: 1, def: 3, group: 'Grenades' },
  { key: 'grenadeRange', label: 'Grenade range (cells)', min: 2, max: 16, step: 1, def: 6, group: 'Grenades' },
  { key: 'grenadeMinRange', label: 'Grenade minimum range', min: 0, max: 8, step: 1, def: 2, group: 'Grenades' },
  { key: 'grenadeAttack', label: 'Grenade damage', min: 0, max: 99, step: 1, def: 6, group: 'Grenades' },
  { key: 'grenadeSplash', label: 'Grenade splash radius', min: 0.5, max: 5, step: 0.1, def: 1.5, group: 'Grenades' },
  { key: 'grenadeCooldown', label: 'Seconds between grenades', min: 0.5, max: 10, step: 0.1, def: 3, group: 'Grenades' },
  // Armor absorbs a share of every hit until its pool is spent; a parachute survives lethal falls
  { key: 'armorPool', label: 'Armor: total damage absorbed', min: 1, max: 999, step: 1, def: 20, group: 'Armor & parachute' },
  { key: 'armorMitigation', label: 'Armor: share of each hit absorbed', min: 0.05, max: 1, step: 0.05, def: 0.5, group: 'Armor & parachute' },
  { key: 'parachuteCharges', label: 'Parachute: lethal falls survived', min: 1, max: 99, step: 1, def: 3, group: 'Armor & parachute' },
  // Guards
  { key: 'guardHpScale', label: 'Guard HP ×', min: 0.1, max: 10, step: 0.1, def: 1, group: 'Guards' },
  { key: 'guardDamageScale', label: 'Guard damage ×', min: 0.1, max: 10, step: 0.1, def: 1, group: 'Guards' },
  { key: 'guardRangeScale', label: 'Guard range ×', min: 0.1, max: 4, step: 0.1, def: 1, group: 'Guards' },
